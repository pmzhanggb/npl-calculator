...
test("recoveryRates: 5 年月回收率按 multiplier 缩放 (内部 ×3 换算季率)", () => {
  // 月率 [1, 2, 3, 4, 5]% → 季率 [3%, 6%, 9%, 12%, 15%] → multiplier=2 → [0.06, 0.12, ...]
  const r = recoveryRates({ year1MonthlyRecovery: 1, year2MonthlyRecovery: 2,
                            year3MonthlyRecovery: 3, year4MonthlyRecovery: 4,
                            year5MonthlyRecovery: 5 }, 2);
  // 浮点容差 (1/100 * 3 * 2 = 0.06, 但 JS 实际 0.06000000000000001)
  r.forEach((v, i) => assert.ok(Math.abs(v - [0.06, 0.12, 0.18, 0.24, 0.30][i]) < 1e-9,
    `index ${i}: ${v} !== expected`));
});
...
test("AMC 优先级: 当 AMC 本金一次性清偿, 先付完所有利息再付本金", () => {
  ...
  const r = project({
    ...
    rebateRate: 0,
    monthlyOverhead: 0,
    year1MonthlyRecovery: 16.67,  // Q1 收 50% 季 = 500万 → 一次性还清 AMC 100万 + 大笔留底
    year2MonthlyRecovery: 0,
    year3MonthlyRecovery: 0,
    year4MonthlyRecovery: 0,
    year5MonthlyRecovery: 0,
  });
  // AMC 配资 = 100万
...
test("AMC 优先级: 资金不足时, AMC 本金部分清偿", () => {
  const r = project({
    ...
    rebateRate: 0,
    monthlyOverhead: 0,
    year1MonthlyRecovery: 0.1667,  // Q1 季回收 0.5% = 0.1667% 月 → 5万 → 不够付利息+本金
    year2MonthlyRecovery: 0,
    year3MonthlyRecovery: 0,
    year4MonthlyRecovery: 0,
    year5MonthlyRecovery: 0,
  });
  // AMC 配资 = 100万, Q1 应收利息 = 2万, 可分配 ≈ 5万 (0.1667% 月 ≈ 0.5% 季, 浮点 ≈ 50010)
  // → amcPaid ≈ 5万 = 2万利息 + 3万本金（先息后本, 浮点 30010）
  assert.ok(Math.abs(r.rows[0].amcInterestPaid - 20000) < 0.01);
  assert.ok(Math.abs(r.rows[0].amcPrincipalPaid - 30010) < 0.01);
  // Q1 后 AMC 余额 ≈ 100 - 3.001 = 96.999 万
  assert.ok(Math.abs(r.rows[0].amcBalance - 969990) < 0.01);
...
test("Mezz 劣后刚性兑付: 可分配额为 0 时, Mezz 利息仍照扣劣后", () => {
  const r = project({
    ...
    rebateRate: 0,
    monthlyOverhead: 0,
    year1MonthlyRecovery: 0,    // Q1..Q20 全 0 回收
    year2MonthlyRecovery: 0,
    year3MonthlyRecovery: 0,
    year4MonthlyRecovery: 0,
    year5MonthlyRecovery: 0,
...
test("Mezz 本金兑付: 仅从可分配额兑付（先还本，区别于 AMC）", () => {
  const r = project({
    ...
    rebateRate: 0,
    monthlyOverhead: 0,
    year1MonthlyRecovery: 1.6667,  // Q1 季回收 5% = 1.6667% 月 → 50万
    year2MonthlyRecovery: 0,
    year3MonthlyRecovery: 0,
    year4MonthlyRecovery: 0,
    year5MonthlyRecovery: 0,
  ...
  // Q1 收 ≈ 1000万 * 5% = 50万 (1.6667% 月 × 3 = 5.0001% 季, 浮点 ≈ 500010)
  // Q1 mezzPrincipalPaid ≈ 500010 (浮点)
  assert.ok(Math.abs(r.rows[0].mezzPrincipalPaid - 500010) < 0.01);
  assert.equal(r.rows[0].mezzInterestDue, 0);
  // Q1 后 Mezz 剩余本金 ≈ 49.999 万
  assert.ok(Math.abs(r.rows[0].mezzBalance - 499990) < 0.01);
...
test("IRR: 劣后回款远低于投入时 IRR 为负", () => {
  ...
  const r = project({
    ...defaults,
    year1MonthlyRecovery: 0.03,  // 0.1/3 ≈ 0.03
    year2MonthlyRecovery: 0.02,  // 0.05/3 ≈ 0.02
    year3MonthlyRecovery: 0.01,  // 0.02/3 ≈ 0.01
    year4MonthlyRecovery: 0,
    year5MonthlyRecovery: 0,
...
test("classify: 全部满足条件 → 可以买 (good)", () => {
  ...
  const r = project({
    ...defaults,
    year1MonthlyRecovery: 1.67,  // 5% 季 → 1.67% 月
    year2MonthlyRecovery: 1.67,
    year3MonthlyRecovery: 1.67,
    year4MonthlyRecovery: 1.67,
    year5MonthlyRecovery: 1.67,
    targetIrr: 15,
  });
...
test("findBreakEvenMultiplier: 永远亏损的参数 → 返回接近 5 的高倍率", () => {
  const v = { ...defaults, year1MonthlyRecovery: 0.003 };  // 0.01%/3 ≈ 0.003% 月
  const be = findBreakEvenMultiplier(v);
  // 极低回收, 需要 5x 才能勉强回本
  assert.ok(be > 1);
});
...
test("classify: watch 分支（盈利但 IRR 未达标）", () => {
  ...
  const r = project({
    ...defaults,
    year1MonthlyRecovery: 1,  // 3% 季 → 1% 月
    year2MonthlyRecovery: 1,
    year3MonthlyRecovery: 1,
    year4MonthlyRecovery: 1,
    year5MonthlyRecovery: 1,
  });
...
test("migrateSnapshotValues: 旧字段名季率 → 新字段月率（季率 /3）", () => {
  const oldValues = {
    faceValue: 100000000,
    purchaseDiscount: 2.8,
    year1QuarterlyRecovery: 1.05,   // 季率
    year2QuarterlyRecovery: 0.72,
    year3QuarterlyRecovery: 0.48,
    year4QuarterlyRecovery: 0.30,
    year5QuarterlyRecovery: 0.18,
  };
  const { values, migrated } = migrateSnapshotValues(oldValues);
  assert.equal(migrated, true);
  assert.equal(values.year1MonthlyRecovery, 0.35);  // 1.05/3 = 0.35
  assert.equal(values.year2MonthlyRecovery, 0.24);
  assert.equal(values.year3MonthlyRecovery, 0.16);
  assert.equal(values.year4MonthlyRecovery, 0.10);
  assert.equal(values.year5MonthlyRecovery, 0.06);
  // 旧字段已删除
  assert.equal(values.year1QuarterlyRecovery, undefined);
  assert.equal(values.year2QuarterlyRecovery, undefined);
  // 其他字段保留
  assert.equal(values.faceValue, 100000000);
  assert.equal(values.purchaseDiscount, 2.8);
});

test("migrateSnapshotValues: 已经是新格式 → 不变（migrated=false）", () => {
  const newValues = {
    faceValue: 100000000,
  ...
  const { values, migrated } = migrateSnapshotValues(newValues);
  assert.equal(migrated, false);
  assert.equal(values.year1MonthlyRecovery, 0.35);
});

test("migrateSnapshotValues: 新旧字段同时存在 → 保留新字段, 删除旧字段", () => {
  const mixed = {
    year1QuarterlyRecovery: 1.05,  // 旧
    year1MonthlyRecovery: 0.50,    // 新 (优先)
  };
  const { values, migrated } = migrateSnapshotValues(mixed);
  assert.equal(migrated, true);
  assert.equal(values.year1MonthlyRecovery, 0.50);  // 新字段保留原值
  assert.equal(values.year1QuarterlyRecovery, undefined);
});

test("migrateSnapshotValues: 部分字段迁移（兼容部分保存）", () => {
  const partial = {
    faceValue: 100000000,
  ...
  const { values, migrated } = migrateSnapshotValues(partial);
  assert.equal(migrated, true);
  assert.equal(values.year1MonthlyRecovery, 0.35);
  assert.equal(values.year5MonthlyRecovery, 0.06);
  assert.equal(values.year2MonthlyRecovery, undefined);  // 原本就缺失
  assert.equal(values.year3MonthlyRecovery, undefined);
});

test("V1.3 业务数据一致: defaults 应与 V1.2 季率口径完全一致", () => {
  // V1.2 defaults: 季率 1.05/0.72/0.48/0.30/0.18
  // V1.3 defaults: 月率 0.35/0.24/0.16/0.10/0.06, 内部 ×3 = 季率 1.05/0.72/0.48/0.30/0.18
  // 两者业务结果应完全一致（浮点容差 10 元）
  const r = project(defaults);
  assert.ok(Math.abs(r.amcNetProfit - 1108516) < 10,
    `AMC 净收益 V1.3 应=¥1,108,516, 实际 ¥${r.amcNetProfit.toFixed(2)}`);
  assert.ok(Math.abs(r.mezzNetProfit - 58800) < 10,
    `Mezz 净收益 V1.3 应=¥58,800, 实际 ¥${r.mezzNetProfit.toFixed(2)}`);
  assert.ok(Math.abs(r.equityProfit - (-359131)) < 10,
    `Equity 净收益 V1.3 应=-¥359,131, 实际 ¥${r.equityProfit.toFixed(2)}`);
});